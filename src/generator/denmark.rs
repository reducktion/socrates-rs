...
use crate::{Citizen, generator::CountryIdGenerator};
...
impl CountryIdGenerator for DenmarkGenerator {
    fn generate(&self, citizen: &Citizen) -> String {
        ...
        let check_digit = if citizen.gender == 'F' { "2".to_string() } else { "3".to_string() };
...
fn is_required_data_present(citizen: &Citizen) -> bool {
    (citizen.gender == 'M' || citizen.gender == 'F') && citizen.year_of_birth > 1800
        && citizen.month_of_birth.is_some() && citizen.day_of_birth.is_some()
}
...
mod tests {
    ...
    fn generate_citizen() {
        ...
        assert_eq!("160691-3113", generator.generate(&Citizen {
            gender: 'M',
            year_of_birth: 1991,
        ...
        assert_eq!("081084-3012", generator.generate(&Citizen {
            gender: 'F',
            year_of_birth: 1984,
